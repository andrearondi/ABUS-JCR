...
def main() -> int:
    ...
    print(f"# [D3] post-hoc selection for {run} — linked val CPM @ op={args.op_thresh} "
          f"(min_epoch={args.min_epoch}, n_val={len(val_ids)}, gt={gt_split})\n")
    hdr = f"{'epoch':>6} {'CPM':>8} {'ceiling':>8} {'recall':>8} {'cands/vol':>10}"
    ...
    for ck in epoch_ckpts:
        ...
        print(f"{e:>6} {cpm_v:>8.4f} {ceil_v:>8.4f} {rec:>8.4f} {pool:>10.1f}", flush=True)

    best_e = select_epoch(epoch_cpms, args.min_epoch)
    ...
    print(f"\nSELECTED epoch {best_e}: linked val CPM = {rows[best_e]['cpm']:.4f} "
          ...
          f"cands/vol {rows[best_e]['cands_per_vol']:.1f}")
